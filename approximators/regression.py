...
class RegressionEstimator(BaseShapleyInteractions):
    ...
    def __init__(self, N, max_order, interaction_type="SII"):
        min_order = 1
        super().__init__(N, max_order, min_order=min_order)
        self._big_M = float(10_000_000)
        assert interaction_type in ("FSI", "SII")
    ...
    def get_S_and_values(budget, num_players, weight_vector, N, pairing, game_fun):
        ...
        for complete_subset in complete_subsets:
            ...
            for subset in combinations:
                ...
                kernel_weights[tuple(sorted(subset))] = weight_vector[len(subset)] / binom(
        ...
        if len(incomplete_subsets) > 0:
            ...
            while len(sampled_subsets) < budget:
                ...
                if sampled_subset not in sampled_subsets:
                    ...
                    kernel_weights_sampling[sampled_subset] = 1.0
                else:
                    kernel_weights_sampling[sampled_subset] += 1.0
                n_sampled_subsets += 1
                if pairing:
                    if len(sampled_subsets) < budget:
                        ...
                        if sampled_subset_paired not in sampled_subsets:
                            ...
                            kernel_weights_sampling[sampled_subset_paired] = 1.0
                        else:
                            kernel_weights_sampling[sampled_subset_paired] += 1.0
                        n_sampled_subsets += 1
            ...
            kernel_weights_sampling = {
                subset: weight * (weight_left / n_sampled_subsets)
                for subset, weight in kernel_weights_sampling.items()
            }
        ...
        for i, S in enumerate(all_S):
            ...
            for s in range(1, self.s_0 + 1):
                ...
                if self.interaction_type == "SII":
                    for combination in powerset(self.N, s, s):
                        index = player_indices[combination]
                        D, A = set(combination), set(S)
                        r = len(A.intersection(D))
                        r_prime = len(D)
                        new_S_value = self._get_bernoulli_weights(r=r, r_prime=r_prime)
                        new_S[i, index] = new_S_value
    ...
    def _get_bernoulli_weights(self, r, r_prime):
        # r: size of intersection S \cap T
        # r_prime: size of interaction |S|
        weight = 0
        for l in range(1, r + 1):
            weight += binom(r, l) * self._bernoulli_numbers[r_prime - l]
        return weight
