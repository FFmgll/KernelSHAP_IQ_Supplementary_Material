...
class SHAPIQEstimator(BaseShapleyInteractions):
    ...
    def compute_interactions_from_budget(
        ...
        if budget > 0:
            ...
            if len(incomplete_subsets) > 0:
                ...
                subset_sizes_samples = random.choices(
                ...
                if stratification:
                    ...
                    for k in subset_sizes_samples:
                        ...
                        if pairing:
                            ...
                            if show_pbar:
                                pbar.update(1)
                        if n_samples[k] > 1:
                            self.result_sample_variance[k] = self.scale_results(
                                result_sample_s2[k], 1 / (n_samples[k] - 1)
                            )

                    for k in incomplete_subsets:
                        result_complete = self.update_results(
                            result_complete, result_sample_mean[k]
                        )
                    ...
                    for k in subset_sizes_samples:
                        ...
                        if n_samples > 1:
                            self.result_sample_variance = self.scale_results(
                                result_sample_s2, 1 / (n_samples - 1)
                            )
    ...
    def compute_interactions_complete(
        self, game: Callable[[set], float], interaction_subsets: dict = None
    ) -> Dict[int, np.ndarray]:
    ...
    def _kernel_q(self, t, sampling_kernel) -> tuple:
        ...
        if sampling_kernel == "ksh":
            if t >= self.s_0 and t <= self.n - self.s_0:
                size_weight = (
                    np.math.factorial(self.n - t - self.s_0)
                    * np.math.factorial(t - self.s_0)
                    / np.math.factorial(self.n - self.s_0 + 1)
                )
            else:
        ...
        if sampling_kernel == "faith":
            if t >= 1 and t <= self.n - 1:
                size_weight = (
                    np.math.factorial(self.n - t - 1)
                    * np.math.factorial(t - 1)
                    / np.math.factorial(self.n - 1)
                )
            else:
                size_weight = self.inf
    ...
    def _kernel_m(self, t, s) -> float:
        ...
        if self.interaction_type == "SII":
            return (
                np.math.factorial(self.n - t - s)
                * np.math.factorial(t)
                / np.math.factorial(self.n - s + 1)
            )
        ...
        if self.interaction_type == "FSI":
            if s == self.s_0:
                return (
                    np.math.factorial(2 * s - 1)
                    / np.math.factorial(s - 1) ** 2
                    * np.math.factorial(self.n - t - 1)
                    * np.math.factorial(t + s - 1)
                    / np.math.factorial(self.n + s - 1)
                )
            else:
    ...
    def _compute_interactions_complete_k(
        self, game: Callable, k: int, pbar: tqdm = None
    ) -> Dict[int, np.ndarray]:
    ...
    def _compute_interactions_complete_k_one(
        self, game: Callable, k: int, interaction: set
    ) -> Dict[set, float]:
    ...
    def compute_interactions_from_budget_one(
        self, game, budget, interaction, pairing=False, sampling_kernel="ksh", sampling_only=False
    ) -> Dict[set, np.ndarray]:
        ...
        if budget > 0:
            ...
            for k in complete_subsets:
                # compute all deterministic subset sizes
                result_complete = self.update_results(
                    result_complete, self._compute_interactions_complete_k_one(game, k, interaction)
                )
            ...
            if len(incomplete_subsets) > 0:
                ...
                for k in subset_sizes_samples:
                    ...
                    if pairing:
                        T_c = self.N - T
                        k_c = len(T_c)
                        result_sample_update = self._evaluate_subset_one(
                            game, T_c, r[k_c], interaction
                        )
                        result_sample_mean, result_sample_s2, n_samples = self.update_mean_variance(
                            result_sample_mean, result_sample_s2, n_samples, result_sample_update
                        )
    ...
    def compute_from_samples(
        self, S_list: List[set], game_values: List, val_empty, val_full, epsilon_correction=True
    ) -> Dict[int, np.ndarray]:
    ...
    def _convert_n_shapley_values_to_one_dimension(
        self, n_shapley_values, n: int = None
        ...
        result_pos = {order: {player: 0.0 for player in range(self.n)} for order in range(1, n + 1)}
        result_neg = {order: {player: 0.0 for player in range(self.n)} for order in range(1, n + 1)}
        result_values = {
            order: {player: [] for player in range(self.n)} for order in range(1, n + 1)
        }
